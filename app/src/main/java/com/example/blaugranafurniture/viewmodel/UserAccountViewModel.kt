...
import com.example.blaugranafurniture.BlaugranaApplication
...
class UserAccountViewModel @Inject constructor(
    ...
    fun updateUser(user: User, imageUri: Uri?) {
        val areInputsValid = validateEmail(user.email) is RegisterValidation.Success && user.firstName.trim().isNotEmpty() && user.lastName.trim().isNotEmpty()
    ...
    private fun saveUserInformationWithNewImage(user: User, imageUri: Uri) {
        viewModelScope.launch {
            try {
                //Handle image to prevent lag that lead to app crashes
                val imageBitmap = MediaStore.Images.Media.getBitmap(getApplication<BlaugranaApplication>().contentResolver, imageUri)
    ...
    private fun saveUserInformation(user: User, shouldRetrieveOldImage: Boolean) {
        firestore.runTransaction { transaction ->
            val docRef = firestore.collection("user").document(auth.uid!!)
